...
//
// v3.13 更新：使用者回報「點完喜歡後感覺要等幾秒才能點下一個，否則不會
//   偵測」。兩個原因：(1) 收集流程本身會自動點分享按鈕、跳出分享選單、
//   點複製連結，這幾秒畫面被分享選單佔用，這段時間手動點下一篇的讚，
//   點到的可能是分享選單而不是真正的讚按鈕——這是設計上的取捨（collectLock
//   序列化收集流程，避免兩個讚同時搶分享按鈕），偵測本身不會漏，只是畫面
//   會被暫時佔用。(2) 重新檢查順便抓到一個真的 bug：watchLikes() 每輪結束
//   會清掉「這輪畫面上看不到的貼文」的 likedSeen 舊紀錄，但分享選單開著
//   的那幾秒，畫面上找不到任何讚按鈕（buttons 是空的），照舊邏輯會把
//   整個 likedSeen 清空——包括那些只是暫時被選單擋住、其實還是已讚狀態
//   的貼文。選單關掉後這些貼文重新出現，紀錄已經沒了，會被誤判成「新的
//   讚」，讓已經收集過的貼文又白白跑一次分享流程（靠 processedUrls 網址
...
//
// v3.15 更新：v3.12 上線後仍回報卡住，這次是靠「往下滑開通知欄」手動
//   解決（不是點「設定」）——通知欄跟「設定」跳出的系統 AlertDialog 是
//   同一類事件，都是貨真價實的系統層級視窗，不是 App 自己疊加的視窗。
//   這說明 v3.8（借用主控台顯示/隱藏）、v3.12（floaty 開一個看不到的空
//   視窗）這兩次的保活戳一下，都不是複製到真正有效的那個機制——兩者都是
//   這支腳本自己向系統要一個疊加視窗，不是系統本身的視窗。改成直接自動化
//   「設定」按鈕本身有效的那個動作：真的跳出一個系統 AlertDialog
//   （dialogs.build()），秀出來立刻程式化關掉，不用手動點——這是目前
...
// 沒有真的複製到「設定」有效的那個機制。
//
// v3.12 改成用 floaty 開一個 1x1、移到螢幕外看不到的空視窗戳一下——一樣
// 是想複製「跳出新視窗」這個訊號，但 v3.14 之後又實測回報卡住（這次靠
// 「往下滑開通知欄」手動解決，不是點「設定」）。往下滑通知欄也是貨真價實
// 的系統層級視窗，跟「設定」跳出的系統 AlertDialog 是同一類事件——這說明
// v3.8（console 顯示/隱藏）、v3.12（floaty 空視窗）這兩次都用錯了機制：
// 兩者都是「這支腳本自己」向系統要一個疊加視窗，不是「系統本身」的視窗；
// 唯一兩次實測真正有效的（設定按鈕、下滑通知欄）都是如假包換的系統
// UI，不是任何 App 能發起的疊加層。
//
// v3.15 改成直接自動化「設定」按鈕本身有效的那個動作——真的跳出一個系統
// AlertDialog（dialogs.build()，不是 floaty 疊加視窗），秀出來立刻程式化
// 關掉，不用你手動點。這是目前唯一真正驗證過有效的機制，不是又一次猜新的
// 替代方案。空對話框（沒有標題/內容/按鈕）理論上只會閃一下，不會真的擋住
// 操作；如果這樣還是會造成明顯的畫面閃爍/打斷，之後可以考慮把
// KEEPALIVE_MS 拉長、用比較低的閃爍頻率換取比較不打斷操作。
//
// 順便把間隔從 15 秒收緊到 8 秒——之前使用者是卡住不到 15 秒就手動處理，
// 沒讓自動保活機制有機會生效就無法判斷它到底有沒有用；縮短間隔至少讓
// 「等一下自動恢復」這個選項在實務上更可行,不用等到快 15 秒那麼久。
...
function watchLikes() {
  ...
  //
  // 但這一輪如果連一顆讚按鈕都沒找到（buttons 是空的），不能直接當「畫面
  // 上的貼文全部消失了」清空整個 likedSeen——實測抓到：收集流程跑到一半
  // 時（點分享按鈕之後），畫面上顯示的是分享選單，不是動態牆本身，這幾秒
  // 內 currentPackage() 仍然是 com.twitter.android、輪詢照樣會跑，但
  // descMatches() 當然找不到任何讚按鈕。如果這裡照樣清空，等分享選單關掉、
  // 貼文重新出現，它的 likedSeen 紀錄已經沒了，會被誤判成「新的讚」，讓
  // 已經收集過的貼文又白白跑一次分享/複製連結流程（靠 processedUrls 網址
  // 去重不會真的送出重複資料，但會多佔用一次 collectLock 排隊時間，讓
  // 手動連續按讚時感覺要等更久）。空清單代表「這輪沒有可靠的畫面讀數」，
  // 不代表「貼文真的都不見了」，跳過這次的清理，等下一輪有讀到東西再清。
  if (buttons.length > 0) {
