...
// 真的有新視窗出現才會恢復。
//
// 改成每 KEEPALIVE_MS 自動戳一下，不用你手動點「設定」，也不用跳出真的
// 會擋畫面的對話框——借用主控台視窗本身（console.show()/console.hide()）
// 開一下立刻關掉，效果應該跟跳出設定對話框一樣（都是疊加視窗的出現/
// 消失），但幾乎感覺不到閃爍，也不會真的要你手動處理什麼。只在主控台
// 目前是隱藏狀態才戳——本來就顯示中代表你正在看 log 除錯，不要沒事把它
// 閃一下打斷畫面。
//
// 這是照這次實測回報推出的 workaround，不是查到 AutoJs6 官方文件記載的
// 行為，真正的底層原因（無障礙節點查詢快取久了會卡住）沒有進一步查證。
// 如果調了之後還是會卡住，回報卡住當下持續了多久、KEEPALIVE_MS 可能要
// 調更短；如果完全沒用，代表這個假設是錯的，要往別的方向查。
  ...
  while (true) {
    ...
    try {
      if (currentPackage() === "com.twitter.android") {
        ...
        if (!consoleVisible && now - lastKeepaliveAt >= KEEPALIVE_MS) {
          lastKeepaliveAt = now;
          console.show();
          sleep(50);
          console.hide();
