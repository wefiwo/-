...
四個 Slash Command，從**你自己在 X／Instagram／Facebook 按讚蒐集的收藏**裡管理貼文：`/抓圖`（角色 autocomplete，留空則隨機挑一位角色 + 類型：圖片/影片，選填數量、選填作者篩選）隨機挑一則貼到 Discord，附上原貼文連結與繪師帳號，回覆下面還有個 🗑️ 按鈕可以直接刪掉這一則；`/抓圖清單` 看某角色收藏了哪些、共幾張（最新加入的排最前面）；`/抓圖刪除` 把收錯的一筆從收藏裡拿掉（網址欄位會自動帶出該角色目前收藏的貼文供選）；`/抓圖統計 [角色]` 看全部角色的收藏概覽，填角色還會多看那個角色目前收藏幾張、排名第幾。User Install 架構，私訊、群組、伺服器都能用，不需要把 bot 拉進伺服器。
